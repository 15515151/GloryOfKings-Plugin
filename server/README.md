...
- **共享只管「主动查询指令」**。`#查询战绩`、`#排位表现` 这类当场发的指令认共享数据；
  `#排位排名`、`#谁在打游戏`、群日报周报、战绩推送一律只认本机绑定。
  因为服务端存的是哈希，没法回答「这群人里谁在库里」。
- **撤销有延迟**。别的机器人那边最多 2 小时后才看不到你的数据。
...
默认监听 `127.0.0.1:8787`。**别直接把它暴露到公网跑 HTTP**——token 会明文过网络。
...
[Service]
Type=simple
WorkingDirectory=/root/Yunzai/plugins/GloryOfKings-Plugin/server
...
服务自己只跑 HTTP，HTTPS 交给反代。Nginx 示例：
...
> **⚠️ 反代场景必读**：服务默认监听回环时会**自动信任** `X-Forwarded-For`
> （因为不这样，所有请求的来源 IP 都变成反代那一个，按 IP 的限流会退化成全局限流，
> 一个 120 次/分钟的桶会把整个服务卡死）。
>
> 反过来，如果监听 `0.0.0.0` 直连公网，默认**不信任**这个头——不然谁都能伪造它绕过限流。
...
| `GOK_HOST` | `127.0.0.1` | 监听地址 |
| `GOK_PORT` | `8787` | 监听端口 |
| `GOK_DB` | `./data/share.db` | SQLite 文件路径 |
| `GOK_TRUST_PROXY` | 跟随监听地址 | 是否信任 `X-Forwarded-For` |
| `GOK_READ_COOLDOWN_MS` | `60000` | 同一 client 重复查同一 QQ 的响应冷却，`0` = 关闭 |
