 ...
 */
async function sendMaster (message) {
  ...
  try {
    ...
    logger.warn(`[${PluginName}] 共享库提醒发送失败：${error?.message || error}`)
    return false
  }
}
...
function buildMessage () {
  return [
    ...
    '',
    '怎么开：共享库得自己搭一个（代码在插件目录 server/ 下，也有部署说明），',
    '或者用朋友搭好的。拿到地址和令牌之后：',
    '  #营地共享库地址 <地址>',
    '  #营地共享库令牌 <令牌>',
    '  #接入营地共享库',
    '',
    '两点要先知道：',
    '· 用户默认**不共享**，要他们自己发 #开启营地ID共享 才会传，你不开这个功能也不影响任何现有功能',
    '· 本机得有一个可用的全局账号，共享过来的营地ID 才查得动（#营地wx全局登录）',
    '',
    '不想用就当没看见，或者发 #关闭营地共享库 让我以后别再问。',
